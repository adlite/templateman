...
export default class Config {
    ...
    if (!cosmiconfig) {
      throw new Error(
        `Templateman configuration file is not found. Please create it.\n${
          chalk.cyan('See documentation on ') + chalk.cyan.underline('https://www.npmjs.com/package/templateman')
        }`,
      );
    }
  ...
  validate() {
    if (!isObject(this.options) || !Array.isArray(this.options.templates)) {
      throw new Error('Templateman config scheme is wrong. It should contain required "templates" array.');
    }
